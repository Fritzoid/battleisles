...
impl std::fmt::Display for MapErrors {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MapErrors::IfOneColumnThenOnlyOneRow => {
                write!(f, "If there is only one column, there can only be one row")
            }
        }
    }
}
            ...
            for col in 0..cols_in_row {
                ...
                match (row, col) {
                    ...
                    (_, 0) => {
                        ...
                        hexes[current_idx - ((collumns as usize) - 1)].neighbors[3] =
                            Some(current_idx);
                        if is_odd_row {
                            hex.neighbors[5] = Some((current_idx - (collumns as usize)) as usize);
                            hexes[current_idx - (collumns as usize)].neighbors[2] =
                                Some(current_idx);
                        }
                    }
                    (_, _) => {
                        if col != collumns - 1 {
                            hex.neighbors[0] =
                                Some((current_idx - ((collumns as usize) - 1)) as usize);
                            hexes[(current_idx - ((collumns as usize) - 1)) as usize].neighbors
                                [3] = Some(current_idx);
                        }
                        hex.neighbors[5] = Some(current_idx - (collumns as usize));
                        hexes[(current_idx - (collumns as usize)) as usize].neighbors[2] =
                            Some(current_idx);
...
mod tests {
    ...
    fn test_map_creation_positions(
        #[case] rows: usize,
        #[case] collumns: usize,
        #[case] expected_positions: &[(i32, i32)],
    ) {
    ...
    fn test_map_creation_neighbors() {
        ...
        assert_eq!(
            sut.hexes[0].neighbors,
            [None, Some(1), Some(3), None, None, None]
        );
        assert_eq!(
            sut.hexes[1].neighbors,
            [None, Some(2), Some(4), Some(3), Some(0), None]
        );
        assert_eq!(
            sut.hexes[2].neighbors,
            [None, None, None, Some(4), Some(1), None]
        );
        assert_eq!(
            sut.hexes[3].neighbors,
            [Some(1), Some(4), Some(6), Some(5), None, Some(0)]
        );
        assert_eq!(
            sut.hexes[4].neighbors,
            [Some(2), None, Some(7), Some(6), Some(3), Some(1)]
        );
        assert_eq!(
            sut.hexes[5].neighbors,
            [Some(3), Some(6), None, None, None, None]
        );
        assert_eq!(
            sut.hexes[6].neighbors,
            [Some(4), Some(7), None, None, Some(5), Some(3)]
        );
        assert_eq!(
            sut.hexes[7].neighbors,
            [None, None, None, None, Some(6), Some(4)]
        );
    }
}
